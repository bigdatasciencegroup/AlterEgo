...
surrounding = config.surrounding
...
for data_file in input_data:
    if data_file['type'] == 'phonemes':
        if 'train' in data_file['filename']:
            train_file = data.process_scrambled(data_file['labels'], [abs_path+"/"+data_file['filename']], channels=config.channels, sample_rate=config.sample_rate, surrounding=config.surrounding, exclude=set([]), num_classes=400)
            print(len(train_file))
            training_files.append(train_file)
        if 'test' in data_file['filename']:
            test_file = data.process_scrambled(data_file['labels'], [abs_path+"/"+data_file['filename']], channels=config.channels,
                                       sample_rate=config.sample_rate, surrounding=config.surrounding,
                                       exclude=set([]), num_classes=400)
...
validation_sequence_groups = data.combine(test_files)
...
log_name = "{}_e{}_b{}_phon_bidir".format(timeString, num_epochs, batch_size)
