...
def data():
    ...
    for data_file in input_data:
        if 'sentences_nora' in data_file['type']:
            if not 'nora_5' in data_file['filename']:
                train_file = data_proc.process_scrambled(data_file['labels'],
                                                         [config.file_path + data_file['filename']],
                ...
                test_file = data_proc.process_scrambled(data_file['labels'], [config.file_path + data_file['filename']],
                                                        channels=config.channels,
                                                        sample_rate=config.sample_rate, surrounding=config.surrounding,
                                                        exclude=set([]), num_classes=config.num_samples)
...
def create_model(x_train, y_train, x_test, y_test):
    ...
    activation = {{choice(['softmax', 'tanh', 'relu', 'sigmoid'])}}
    ...
    encoder_inputs = Input(shape=(2450, len(config.channels)))
