...
from tensorflow.python.keras.layers import Input, CuDNNLSTM, LSTM, Dense
...
for i in range(num_encoder_layers):
    ...
    if i < num_encoder_layers - 1:
        encoder_lstm = CuDNNLSTM(latent_dim, return_sequences=True, return_state=True)(inputs)
    else:
        encoder_lstm = CuDNNLSTM(latent_dim, return_state=True)(inputs)
...
for i in range(num_decoder_layers):
    if i == 0:
        decoder_lstm = CuDNNLSTM(latent_dim, return_sequences=True, return_state=True)(decoder_inputs, initial_state=encoder_states)
    else:
        decoder_lstm = CuDNNLSTM(latent_dim, return_sequences=True, return_state=True)(decoder_lstm)
...
log_name = "{}_e{}_b{}_phon".format(timeString, num_epochs, batch_size)
