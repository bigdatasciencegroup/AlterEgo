...
from tensorflow.python.keras.layers import Input, CuDNNLSTM, LSTM, Dense, Concatenate, Bidirectional, Dropout
...
# Print header
result_file.write('# HYPERPARAMETERS:\nepochs:{}\nbatch size:{}\nlatent dim:{}\nlearning rate:{}\ndecay:{}\nattention:{}\nearly stopping:{}\nfolds:{}\nencoder dropout rate:{}\nencoder recurrent dropout rate:{}\ndecoder dropout rate:{}\ndecoder recurrent dropout rate:{}\noutput dropout rate:{}\n'.format(config.num_epochs, config.batch_size, config.latent_dim, config.learning_rate, config.decay, config.with_attention, config.early_stopping, config.num_folds, config.enc_dropout_rate, config.enc_recurrent_dropout_rate, config.dec_dropout_rate, config.dec_recurrent_dropout_rate, config.output_dropout_rate))
...
for fold in list(range(config.num_folds)):
    ...
    decoder_outputs = Dropout(config.output_dropout_rate)(decoder_outputs)
