...
#### Load data
def dataset(**kwargs):
    filename = 'patient_data/sara/sara.txt'
    labels = [6, 6, 2, 5, 5, 3, 6, 3, 4, 5, 3, 2, 8, 3, 7, 0, 2, 4, 3, 8, 3, 2, 3, 1, 5,\
    8, 6, 9, 2, 6, 0, 0, 0, 8, 1, 7, 8, 6, 3, 4, 1, 1, 2, 1, 4, 5, 7, 0, 8, 5,\
    1, 9, 0, 4, 8, 0, 1, 5, 3, 2, 0, 1, 6, 7, 9, 5, 2, 4, 4, 9, 9, 9, 9, 7, 8,\
    9, 0, 4, 2, 7, 5, 7, 1, 6, 9, 5, 8, 2, 8, 0, 7, 6, 1, 3, 9, 6, 7, 4, 7, 4,\
    13, 11, 10, 11, 12, 12, 13, 11, 12, 14, 13, 10, 11, 14, 13, 12, 14, 10, 13, 14, 10, 10, 12, 11, 12,\
    10, 13, 13, 13, 10, 10, 12, 10, 14, 11, 12, 12, 14, 11, 13, 11, 11, 14, 14, 14, 11, 13, 10, 12, 14]
    return data.process_scrambled(labels, [filename], **kwargs)

# channels = range(1, 8) # DO NOT CHANGE
channels = range(0, 8)

total_data = dataset(channels=channels, surrounding=260)
print np.array(total_data).shape # (15,10)
sequence_groups = transform_data(total_data)
print len(sequence_groups) # 15
...
# sequence_groups = data.transform.pad_truncate(sequence_groups, length) # sorted labels
# # print sequence_groups
# sequences, labels = data.get_inputs(sequence_groups)
# print labels
# [1 9 7 1 13 5 14 4 2 12 9 0 10 5 7 4 9 6 3 3 9 8 7 4 12 9 3 1 7 14 9 13 6
# 12 11 4 5 2 5 10 11 1 11 10 5 11 1 0 4 5 3 3 11 13 10 13 12 8 12 9 1 11
# 12 1 0 10 0 12 11 6 5 14 13 4 6 7 11 9 10 8 1 3 3 12 3 8 11 14 2 2 6 0 13
# 9 10 14 8 14 10 6 1 8 4 8 5 2 7 4 2 13 12 14 4 2 7 0 14 3 14 10 4 11 1 8
# 0 8 6 0 0 5 2 6 6 0 8 9 6 5 10 2 2 13 7 12 7 13 13 7 14 3]
...
for fold in range(5):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as session:
        ...
        for epoch in range(num_epochs):
            ...
            if validation_accuracy > max_validation_accuracy:
                model_name = 'checkpoints/s_model.ckpt'
                saved_training_accuracy = training_accuracy
    ...
    print 'Training accuracy {:.2f} %'.format(saved_training_accuracy*100), 'Final training accuracy {:.2f} %'.format(training_accuracy*100)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as session:
        tf.global_variables_initializer().run()
        saver.restore(session, 'checkpoints/s_model.ckpt')
