...
def split_seqs(sequence_groups, test_train_split, fold, seed=0):
    np.random.seed(seed)
    ...
    for i in range(len(sequence_groups)):
        if i==fold/10:      test_train_split = (test_train_split*10)/9
...
def split_data(fold, sequence_groups):
    ...
    test_seq_groups = [np.array(sequence_groups[fold/10])[fold%10]]
    test_labels = np.array([fold/10])

    for i in range(len(sequence_groups)):
        if i!=fold/10:
            rest = range(10)
        else:
            rest = list(set(range(10)) - set([fold%10]))
        train_seq_groups[i] = np.array(sequence_groups[i])[rest]
        # print len(train_seq_groups[i])

    train_seq_groups, val_seq_groups = split_seqs(train_seq_groups, 2./10, fold)
    ...
    test_seqs, _ = data.get_inputs(data.transform.pad_truncate(test_seq_groups, length))
    test_seqs = np.reshape(test_seqs, (1, length, len(channels)))
...
for fold in range(150):
    ...
    num_classes = 15
    # print 'Train sequences\t', np.shape(train_sequences) # (119, 900, 8)
    # print 'Train labels\t', np.shape(train_labels) # (119, 15)
    # print 'Val sequences\t', np.shape(val_sequences) # (30, 900, 8)
    # print 'Val labels\t', np.shape(val_labels) # (30, 15)
    # print 'Test sequences\t', np.shape(test_sequences) # (1, 900, 8)
    # print 'Test labels\t', np.shape(test_labels) # (1, )
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as session:
        ...
        for epoch in range(num_epochs):
            ...
            if validation_accuracy > max_validation_accuracy:
                model_name = 'checkpoints/c_model_fold_{}.ckpt'.format(fold)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as session:
        tf.global_variables_initializer().run()
        saver.restore(session, 'checkpoints/c_model_fold_{}.ckpt'.format(fold))
        ...
        for sequence,label in zip(test_sequences, test_labels):
            ...
            # print 'Actual:', label 
            # print
            print 'Predicted {}\tActual {}'.format(np.argmax(test_output), label)
