...
sequence_groups = transform_data(data.process_scrambled(labels, ['eric2.txt'], channels=channels, sample_rate=250))

silence_group = transform_data(data.process(1, ['eric2_silence.txt'], channels=channels, sample_rate=250))
...
training_sequence_groups = data.transform.pad_truncate(training_sequence_groups, 450)
validation_sequence_groups = data.transform.pad_truncate(validation_sequence_groups, 450)
length = 350 # DO NOT CHANGE
...
with tf.Session() as session:
    ...
    for epoch in range(num_epochs):
        ...
        for test_index in range(num_classes):
            if (epoch+1) % 1 == 0:
                test_input = np.random.rand(num_classes*num_copies, length, num_channels) * 2 - 1.0
#                test_input = np.zeros((num_classes*num_copies, length, num_channels))
                for i in range(100):
                    test_targets = np.zeros((num_classes*num_copies, num_classes))
                    for c in range(num_classes-1):
                        test_targets[c*num_copies:(c+1)*num_copies,c] = 1.0
                    gradients_output, logits_output = session.run([gradients, logits],
                                                                  {inputs: test_input, targets: test_targets,
                                                                   weights:np.ones(len(test_targets)), training: False})
