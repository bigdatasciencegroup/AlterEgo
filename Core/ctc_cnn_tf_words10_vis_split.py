...
inputs = tf.placeholder(tf.float32, [None, None, num_channels])
split_inputs = tf.expand_dims(inputs, 3)
split_inputs = tf.extract_image_patches(split_inputs,
                                        ksizes=[1, cnn_window_size, 1, 1],
                                        strides=[1, cnn_window_stride, 1, 1],
                                        rates=[1, 1, 1, 1], padding='VALID')
split_inputs = tf.transpose(split_inputs, [0, 1, 3, 2])
...
max_timesteps = tf.shape(split_inputs)[1]
...
reshaped = tf.reshape(split_inputs, [-1, cnn_window_size, num_channels])
...
with tf.Session() as session:
    ...
    for epoch in range(num_epochs):
        ...
        for batch in range(num_training_batches):            
            ...
            actual_lengths = map(len, batch_sequences)
            batch_lengths = np.array(actual_lengths) // cnn_window_stride
            batch_sequences = data.transform.pad_truncate(batch_sequences, max(actual_lengths), position=0.0)
