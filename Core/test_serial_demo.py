...
with tf.Session(graph=graph1) as session1:
    ...
    saver1.restore(session1, 'math27_trigger_model.ckpt')
    with tf.Session(graph=graph2) as session2:
        tf.global_variables_initializer().run()
        saver2.restore(session2, 'math27_activation_small_model.ckpt')
...
#                    
                    ...
                    if mode_toggle:
                        if time.time() - cycle_timestamp > 1.5:
                            cycle_timestamp = time.time()
                            cycle_num = (cycle_num + 1) % num_classes
                    ...
                    if activated:
                        if not mode_toggle:
                            if len(predictions) < len(intended):
                                best_guess = intended[len(predictions)]
                            ...
                            if time.time() - produced_timestamp > 0.5:
                                best_guess = (cycle_num + num_classes - 1) % num_classes
                                produced_timestamp = time.time()
                                predictions.append(best_guess)
                                os.system('say ' + str(word_map[best_guess]) + ' &')
...
#                            produced_timestamp = time.time()
#                            predictions.append(best_guess)
#                            if best_guess < num_classes-1:
#                                os.system('say ' + str(word_map[best_guess]) + ' &')
#                            elif best_guess == num_classes-1 and predictions[-2] < num_classes-1:
#                                os.system('say ' + str(word_map[predictions[-2]]) + ' confirmed &')
