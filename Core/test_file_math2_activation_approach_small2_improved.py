...
num_classes = 14 #len(filter(lambda x: '.txt' in x, os.listdir('patient_data')))
...
with tf.Session(graph=graph1) as session1:
    if test_model:
        ...
        saver1.restore(session1, 'math2_trigger_model.ckpt')
    with tf.Session(graph=graph2) as session2:
        if test_model:
            tf.global_variables_initializer().run()
            saver2.restore(session2, 'math2_activation_small_model.ckpt')
...
#                    activated = sum(activation_windows) >= len(activation_windows) - 2
                    ...
                    for i in range(len(activation_windows)):
                        if i >= 2 and np.all(activation_windows[i-2:i]) and not activation_windows[i]:
                            if 6 <= i - tmp_start and i < len(activation_windows) - 2: # and i - tmp_start <= 25
...
#                        print
    ...
    #                      history_size=1500, shown_size=1200, override_step=35)
        ...
        data.from_file.start('math2_test.txt',
                             on_data, channels=channels, transform_fn=transform_data,
