...
training_sequence_groups = transform_data(data.process_scrambled(labels, ['math3.txt'], channels=channels, sample_rate=250, surrounding=surrounding))

training_silence_group = transform_data(data.combine([
            data.process(1, ['math3_silence.txt'], channels=channels, sample_rate=250, surrounding=surrounding),
            data.process(1, ['math3_silence2.txt'], channels=channels, sample_rate=250, surrounding=surrounding),
        ]))
...
validation_sequence_groups = transform_data(data.process_scrambled(labels, ['math3_test.txt'], channels=channels, sample_rate=250, surrounding=surrounding))

validation_silence_group = transform_data(data.process(1, ['math3_test_silence.txt'], channels=channels, sample_rate=250, surrounding=surrounding))
    ...
    with tf.Session() as session:
        ...
        for epoch in range(num_epochs):
            ...
            if validation_accuracy > max_validation_accuracy:
                model_name = 'math3_triggerless_model.ckpt'
