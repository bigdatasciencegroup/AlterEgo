...
with tf.Session(graph=graph1) as session1:
    if test_model:
        ...
        saver1.restore(session1, 'math9_trigger_model.ckpt')
    with tf.Session(graph=graph2) as session2:
        if test_model:
            tf.global_variables_initializer().run()
            saver2.restore(session2, 'math9_activation_small_model.ckpt')
    ...
    #                            predictions.append(selected)
                ...
                trigger_labels = [11, 6, 13, 14, 2, 5, 3, 10, 9, 12, 12, 12, 0, 11, 14, 13, 13, 12, 6, 13, 14, 2, 10, 13, 0, 12, 4, 12, 11, 14, 12, 12, 7, 11, 5, 6, 4, 10, 4, 2, 14, 3, 4, 13, 4, 11, 11, 10, 13, 6, 8, 11, 2, 4, 9, 13, 0, 10, 1, 12, 12, 6, 12, 13, 6, 3, 5, 3, 11, 8, 12, 6, 2, 3, 0, 14, 3, 14, 12, 11, 1, 13, 0, 13, 6, 3, 13, 2, 0, 14, 12, 10, 11, 10, 9, 13, 1, 3, 3, 11, 10, 2, 11, 3, 6, 3, 0, 9, 2, 0, 9, 8, 9, 0, 8, 4, 9, 11, 13, 4]
...
#                    activated = np.all(1-np.argmax(activation_output, axis=1) == 1)
                    ...
                    activation_windows = np.array(map(lambda x: x[0] > 0.99, activation_output)) # 0.99999
...
#                    activated = sum(activation_windows) >= len(activation_windows) - 2
                    ...
                    for i in range(len(activation_windows)):
                        if i >= 4 and np.all(activation_windows[i-4:i]) and not activation_windows[i]: # 4
                            if 7 <= i - tmp_start and i < len(activation_windows) - 2: # and i - tmp_start <= 25  # 10<=
...
#                print 'Predicted\t', predictions
        ...
        data.from_file.start('math9_test.txt',
