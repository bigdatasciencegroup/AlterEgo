...
autoencoder_inputs = tf.placeholder(tf.float32,[None, window_size, num_channels])
autoencoder_training = tf.placeholder(tf.bool)

conv1 = tf.layers.conv1d(autoencoder_inputs, 400, 50, activation=tf.nn.relu, padding='valid')
conv1 = tf.layers.max_pooling1d(conv1, 2, strides=2)
conv2 = tf.layers.conv1d(conv1, 400, 25, activation=tf.nn.relu, padding='valid')
conv2 = tf.layers.max_pooling1d(conv2, 2, strides=2)
conv3 = tf.layers.conv1d(conv2, 400, 12, activation=tf.nn.relu, padding='valid')
conv3 = tf.layers.max_pooling1d(conv3, 2, strides=2)
conv4 = tf.layers.conv1d(conv3, 400, 12, activation=tf.nn.relu, padding='valid')
conv4 = tf.layers.max_pooling1d(conv4, 2, strides=2)
conv5 = tf.layers.conv1d(conv4, 400, 12, activation=tf.nn.relu, padding='valid')
conv5 = tf.layers.max_pooling1d(conv5, 2, strides=2)
dropout = tf.layers.dropout(conv5, dropout_rate, training=autoencoder_training)
...
autoencoder_outputs = tf.layers.dense(fc3, window_size * num_channels)
autoencoder_outputs = tf.reshape(autoencoder_outputs, [-1, window_size, num_channels])


saver = tf.train.Saver()
with tf.Session() as autoencoder_session:
    tf.global_variables_initializer().run()
    saver.restore(autoencoder_session, 'autoencoder_word10_1000_model.ckpt')
...
#                print np.shape(batch_sequences)
#                print map(np.shape, batch_sequences)
#                plt.plot(batch_sequences[0][0])
#                plt.show()
                
                batch_sequences = map(lambda sequences:
                                      autoencoder_session.run(autoencoder_outputs,
                                                              {autoencoder_inputs: sequences,
