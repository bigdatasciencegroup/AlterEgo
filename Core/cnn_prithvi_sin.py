...
def transform_data_sin(sequence_groups, period, sample_rate=250):
    #### Apply DC offset and drift correction
    drift_low_freq = 0.5 #0.5
    sequence_groups = data.transform.subtract_initial(sequence_groups)
    sequence_groups = data.transform.highpass_filter(sequence_groups, drift_low_freq, sample_rate)
...
def augment_channels(sequence_groups, sequence_groups2):
    return np.array([np.array([np.concatenate([sequence_groups[i][j], sequence_groups2[i][j]], axis=1)
                               for j in range(len(sequence_groups[i]))]) for i in range(len(sequence_groups))])

# Manually selecting different training and validation datasets
training_sequence_groups = data.digits_session_1_dataset()
t50 = transform_data_sin(training_sequence_groups, 50)
t100 = transform_data_sin(training_sequence_groups, 100)
t150 = transform_data_sin(training_sequence_groups, 150)
t200 = transform_data_sin(training_sequence_groups, 200)
t250 = transform_data_sin(training_sequence_groups, 250)
training_sequence_groups = transform_data(training_sequence_groups)
training_sequence_groups = reduce(augment_channels, [t50, t100, t150, t200, t250], training_sequence_groups)

validation_sequence_groups = data.digits_session_4_dataset()
v50 = transform_data_sin(validation_sequence_groups, 50)
v100 = transform_data_sin(validation_sequence_groups, 100)
v150 = transform_data_sin(validation_sequence_groups, 150)
v200 = transform_data_sin(validation_sequence_groups, 200)
v250 = transform_data_sin(validation_sequence_groups, 250)
validation_sequence_groups = transform_data(validation_sequence_groups)
validation_sequence_groups = reduce(augment_channels, [v50, v100, v150, v200, v250], validation_sequence_groups)
...
fc1 = tf.layers.dense(reshaped, 250, activation=tf.nn.relu, kernel_initializer=tf.initializers.truncated_normal(
        stddev=np.sqrt(2.0/(int(np.prod(dropout.shape[1:]))*250))))
logits = tf.layers.dense(fc1, num_classes, activation=tf.nn.softmax,
        kernel_initializer=tf.initializers.truncated_normal(stddev=np.sqrt(2.0/(250 * num_classes))))
