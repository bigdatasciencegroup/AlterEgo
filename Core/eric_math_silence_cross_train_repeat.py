...
training_sequence_groups = transform_data(data.process_scrambled(labels, ['math1.txt'], channels=channels, sample_rate=250, surrounding=surrounding))

training_silence_group = transform_data(data.process(1, ['math1_silence.txt'], channels=channels, sample_rate=250, surrounding=surrounding))
...
validation_sequence_groups = transform_data(data.process_scrambled(labels, ['math1_test.txt'], channels=channels, sample_rate=250, surrounding=surrounding))

validation_silence_group = transform_data(data.process(1, ['math1_test_silence.txt'], channels=channels, sample_rate=250, surrounding=surrounding))
...
length = 500 #320, 410 DO NOT CHANGE
    ...
    with tf.Session() as session:
        ...
        for epoch in range(num_epochs):
            ...
            if training_accuracy > 0.97:
                good_run = True
            if not good_run and epoch + 1 >= 150:
                break
