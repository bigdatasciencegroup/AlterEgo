...
with tf.Session(graph=graph1) as session1:
    if test_model:
        ...
        saver1.restore(session1, 'math7_trigger_model' + sys.argv[2] + '.ckpt')
    with tf.Session(graph=graph2) as session2:
        if test_model:
            tf.global_variables_initializer().run()
            saver2.restore(session2, 'math7_activation_small_model' + sys.argv[2] + '.ckpt')
    ...
    #                            predictions.append(selected)
                ...
                trigger_labels = [11, 6, 7, 14, 9, 5, 3, 10, 6, 12, 12, 12, 0, 11, 14, 13, 13, 12, 8, 7, 14, 2, 10, 13, 9, 12, 7, 5, 11, 14, 1, 5, 7, 8, 1, 6, 4, 10, 1, 2, 14, 3, 7, 13, 4, 8, 11, 10, 5, 6, 5, 11, 2, 4, 9, 13, 0, 10, 1, 12, 5, 6, 5, 13, 6, 3, 5, 3, 11, 9, 12, 6, 2, 8, 2, 14, 3, 14, 5, 11, 1, 7, 0, 13, 7, 2, 7, 7, 0, 14, 1, 10, 12, 10, 8, 13, 12, 3, 3, 1, 10, 2, 12, 2, 6, 3, 0, 8, 9, 0, 2, 5, 9, 0, 9, 1, 9, 1, 13, 7]
...
#                    activated = np.all(1-np.argmax(activation_output, axis=1) == 1)
                    ...
                    activation_windows = np.array(map(lambda x: x[0] > 0.99999, activation_output)) # 0.99999
...
#                    activated = sum(activation_windows) >= len(activation_windows) - 2
                    ...
                    for i in range(len(activation_windows)):
                        if i >= 4 and np.all(activation_windows[i-4:i]) and not activation_windows[i]: # 4
                            if 8 <= i - tmp_start and i < len(activation_windows) - 2: # and i - tmp_start <= 25  # 10<=
                                activated_start, activated_end = tmp_start, i
                        if i >= 5 and not np.any(activation_windows[i-5:i]) and activation_windows[i]:
                            tmp_start = i
...
#                print 'Predicted\t', predictions
                ...
                for i in range(len(predictions)):
                    x1 = str(ground_truths[i] or '')
                    x2 = str(trigger_predictions[i] or '')
                    x3 = str(predictions[i] or '')
        ...
        data.from_file.start('math7_test.txt',
