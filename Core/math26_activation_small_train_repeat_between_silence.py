...
sequence_groups = transform_data([data.flatten(data.process_scrambled(labels, ['math26.txt'], channels=channels, sample_rate=250,
                                                                      surrounding=surrounding, exclude=set([259, 273])))])

#silence_group = transform_data(data.process(1, ['math6_silence.txt'], channels=channels, sample_rate=250, surrounding=surrounding))
silence_group = transform_data(data.process_silence_between(['math26.txt'], channels=channels, sample_rate=250, surrounding=surrounding))
    ...
    with tf.Session() as session:
        ...
        for epoch in range(num_epochs):
            ...
            if validation_accuracy > max_validation_accuracy:
                model_name = 'math26_activation_small_model.ckpt'
