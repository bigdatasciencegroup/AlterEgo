...
sequence_groups = transform_data([data.flatten(data.process_scrambled(labels, ['math3.txt'], channels=channels, sample_rate=250, surrounding=surrounding))])

silence_group = transform_data(data.process(1, ['math3_silence.txt'], channels=channels, sample_rate=250, surrounding=surrounding))
silence_group = data.transform.augment_amplitude_multipliers(silence_group, np.arange(0.1, 2.1, 0.1))
    ...
    with tf.Session() as session:
        ...
        for epoch in range(num_epochs):
            ...
            if validation_accuracy > max_validation_accuracy:
                model_name = 'math3_activation_small_model.ckpt'
