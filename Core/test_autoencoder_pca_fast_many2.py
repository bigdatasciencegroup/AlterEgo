...
#        plt.show()
        ...
        num_cols = 8
        downsample_factor = 1
        fig, axes = plt.subplots(int(math.ceil(float(num_shown*2) / num_cols)), num_cols, figsize=(18, 11))
        plt.subplots_adjust(left=0.03, right=0.99, bottom=0.03, top=0.93, wspace=0.35, hspace=0.35)
        plt.suptitle('Effect of Each Principle Component on Autoencoder Output', y=0.995, fontweight='bold')
        colors = ['gray', 'purple', 'blue', 'green', 'yellow', 'orange', 'red', 'brown']
        lines = [[axes[i//num_cols][i%num_cols].plot([],[], '-', c=colors[c])[0] for c in range(8)] for i in range(num_shown*2)]
        for i in range(0, num_shown*2, 2):
            axes[i//num_cols][i%num_cols].axis([0, window_size//downsample_factor, -125, 125])
            axes[(i+1)//num_cols][(i+1)%num_cols].axis([0, window_size//downsample_factor, -125, 125])
            axes[i//num_cols][i%num_cols].set_title('+PC' + str(i//2+1) + ' (' + str(eigenvalues[i//2]) + ')')
            axes[(i+1)//num_cols][(i+1)%num_cols].set_title('-PC' + str(i//2+1) + ' (' + str(eigenvalues[i//2]) + ')')
...
#            test_encodings = [np.array(flat) for _ in range(num_shown)]
            test_encodings = [np.array(mean) for _ in range(num_shown*2)]
#            test_encodings = [np.array(mu) for _ in range(num_shown)]

#            if last_value == 500:
#                plt.savefig('figure_autoencoder_principle_components_pos.png')
#            if last_value == -500:
#                plt.savefig('figure_autoencoder_principle_components_neg.png')

            value = 500 * np.sin(10 * test_count * np.pi / 180.0)
            last_value = value
            for j in range(num_shown):
                test_encodings[2*j][j] += value
                test_encodings[2*j+1][j] -= value
            print value
            test_count += 1
