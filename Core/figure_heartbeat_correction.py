...
sequence = sequence_groups[1][0]
fft = np.abs(data.transform.fft(sequence)) / len(sequence)
ricker_width = 35 * sample_rate // 250
ricker_sigma = 4.0 * sample_rate / 250
ricker_kernel = normalize_kernel(ricker_wavelet(ricker_width, ricker_sigma))
ricker_convolved = data.transform.correlate(sequence_groups, ricker_kernel)
ricker_subtraction_multiplier = 2.0
sequence_groups = sequence_groups - ricker_subtraction_multiplier * ricker_convolved
ricker_convolved = ricker_convolved[1][0]
sequence2 = sequence_groups[1][0]
fft2 = np.abs(data.transform.fft(sequence2)) / len(sequence2)
#sequence[np.where(sequence > 100)] -= 60
...
plt.subplots_adjust(left=0.05, right=0.995, bottom=0.075, top=0.96, hspace=0.45, wspace=0.4)
#axes[0][0].set_title('Signal with Power Line Interference')
#axes[0][0].set_ylabel(u'Signal (\u03bcV)')
#axes[1][0].set_title('Notch-Filtered Signal (Multiples of 60 Hz Butterworth)')
#axes[1][0].set_xlabel('Sample')
#axes[1][0].set_ylabel(u'Signal (\u03bcV)')
#axes[0][1].set_title('FFT of Signal with Power Line Interference')
#axes[0][1].set_ylabel('Magnitude')
#axes[1][1].set_title('FFT of Notch-Filtered Signal')
#axes[1][1].set_xlabel('Frequency (Hz)')
#axes[1][1].set_ylabel('Magnitude')
#for i in range(len(channels)):
#    axes[0][0].plot(sequence[:,i], c=colors[channels[i]], alpha=1.0, lw=1.0)
#    axes[1][0].plot(sequence2[:,i], c=colors[channels[i]], alpha=1.0, lw=1.0)
#    axes[0][1].plot(np.arange(len(fft)//2+1) * sample_rate/length, fft[:len(fft)//2+1,i],
#                    c=colors[channels[i]], alpha=1.0, lw=1.0)
#    axes[1][1].plot(np.arange(len(fft2)//2+1) * sample_rate/length, fft2[:len(fft2)//2+1,i],
#                    c=colors[channels[i]], alpha=1.0, lw=1.0)
plt.subplot2grid((3, 4), (0, 0), colspan=1, rowspan=3)
plt.plot(ricker_kernel)
plt.title(u'Ricker Wavelet (\u03C3=16)')
plt.xlabel('Index')
plt.subplot2grid((3, 4), (0, 1), colspan=3, rowspan=1)
for i in range(len(channels)):
    plt.plot(sequence[:,i], c=colors[channels[i]], alpha=1.0, lw=1.0)
plt.title('Notch-Filtered Signal')
plt.ylabel(u'Signal (\u03bcV)')
plt.subplot2grid((3, 4), (1, 1), colspan=3, rowspan=1)
for i in range(len(channels)):
    plt.plot(ricker_convolved[:,i], c=colors[channels[i]], alpha=1.0, lw=1.0)
plt.title('Ricker Convolution')
plt.ylabel(u'Signal (\u03bcV)')
plt.subplot2grid((3, 4), (2, 1), colspan=3, rowspan=1)
for i in range(len(channels)):
    plt.plot(sequence2[:,i], c=colors[channels[i]], alpha=1.0, lw=1.0)
plt.title('Notch-Filtered Signal with Heartbeat Artifacts Removed')
plt.ylabel(u'Signal (\u03bcV)')
plt.xlabel('Sample')
plt.savefig('figure_heartbeat_correction.png')
plt.show()
