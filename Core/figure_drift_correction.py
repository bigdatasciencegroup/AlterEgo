...
sequence2 = data.transform.highpass_filter(sequence, drift_low_freq, sample_rate)
sequence2 = data.transform.subtract_mean(sequence2)
...
plt.subplots_adjust(left=0.06, right=0.995, bottom=0.12, top=0.94, hspace=0.45)
axes[0].set_title('Signal with Uncorrected Drift')
#axes[0].set_xlabel('Sample')
axes[0].set_ylabel(u'Signal (\u03bcV)')
axes[1].set_title('Highpassed Signal (0.5Hz Butterworth)')
axes[1].set_xlabel('Sample')
axes[1].set_ylabel(u'Signal (\u03bcV)')
for i in range(len(channels)):
    axes[0].plot(sequence[:,i], c=colors[channels[i]], alpha=1.0, lw=1.0)
    axes[1].plot(sequence2[:,i], c=colors[channels[i]], alpha=1.0, lw=1.0)
plt.savefig('figure_drift_correction.png')
plt.show()
