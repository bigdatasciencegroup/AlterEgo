...
sequence_groups = transform_data(
    data.words_10_20_sentences_dataset(include_surrounding=False, channels=[6]))
...
with tf.Session() as session:
    ...
    saver.restore(session, 'autoencoder_single_' + str(window_size) + '_' + str(latent_size) + '_model.ckpt')
...
#        colors = ['gray', 'purple', 'blue', 'green', 'yellow', 'orange', 'red', 'brown']
        colors = ['red']
        lines = [[axes[i//num_cols][i%num_cols].plot([],[], '-', c=colors[c])[0] for c in range(1)] for i in range(num_shown)]
        for i in range(num_shown):
            axes[i//num_cols][i%num_cols].axis([0, window_size//downsample_factor, -105, 105])
            axes[i//num_cols][i%num_cols].set_title('PC' + str(i+1) + ' (' + str(eigenvalues[i]) + ')')
        flat = [[0.0] * num_channels for _ in range(window_size)]
        
#        plt.figure(figsize=(3, 3))
#        plt.subplots_adjust(left=0.0, right=1.0, bottom=0.0, top=1.0)
#        plt.plot(flat, 'red')
#        plt.gca().set_xlim((0, window_size))
#        plt.gca().set_ylim((-105, 105))
#        filename = 'figure_autoencoder_pca_flat_input.png'
...
#        imsave(filename, image)
        ...
        flat = session.run(fc1, {inputs: [flat], training: False})[0]
        print list(flat)
        flat = pca.transform([flat])[0]
        print list(flat)
...
#            if last_value == 200:
#                plt.savefig('figure_autoencoder_single_principal_components_pos.png')
#            if last_value == -200:
#                plt.savefig('figure_autoencoder_single_principal_components_neg.png')

            value = 200 * np.sin(10 * test_count * np.pi / 180.0)
            last_value = value
            ...
            if last_value == 200:
                print list(test_reconstructeds[4])
...
#                plt.figure(figsize=(3, 3))
#                plt.subplots_adjust(left=0.0, right=1.0, bottom=0.0, top=1.0)
#                plt.plot(reconstructed_outputs[12], 'red')
#                plt.gca().set_xlim((0, window_size))
#                plt.gca().set_ylim((-105, 105))
#                filename = 'figure_autoencoder_pca_flat_output13.png'
#                plt.savefig(filename, transparent=True)
#                plt.show()
#                image = imread(filename)
#                image = image[2:-1,2:-1]
...
#                plt.figure(figsize=(3, 3))
#                plt.subplots_adjust(left=0.0, right=1.0, bottom=0.0, top=1.0)
#                plt.plot(reconstructed_outputs[4], 'red')
#                plt.gca().set_xlim((0, window_size))
#                plt.gca().set_ylim((-105, 105))
#                filename = 'figure_autoencoder_pca_flat_output5.png'
#                plt.savefig(filename, transparent=True)
#                plt.show()
#                image = imread(filename)
#                image = image[2:-1,2:-1]
