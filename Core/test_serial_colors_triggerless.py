...
num_classes = 3 #len(filter(lambda x: '.txt' in x, os.listdir('patient_data')))
...
with tf.Session(graph=graph1) as session1:
    if test_model:
        ...
        saver1.restore(session1, 'colors_trigger_model.ckpt')
    with tf.Session(graph=graph2) as session2:
        if test_model:
            tf.global_variables_initializer().run()
            saver2.restore(session2, 'colors_activation_small_model.ckpt')
...
#        data.serial.start('/dev/tty.usbserial-DQ007UBV',
        data.serial.start('/dev/tty.usbserial-DM01HUN9',
                          on_data, channels=channels, transform_fn=transform_data,
                          history_size=1500, shown_size=1200, override_step=165) #95