...
training_silence_group = transform_data(data.process(1, ['math3_silence.txt'], channels=channels, sample_rate=250, surrounding=surrounding))
...
validation_sequence_groups = transform_data(data.process_scrambled(labels, ['math4.txt'], channels=channels, sample_rate=250, surrounding=surrounding))[:-1]

validation_silence_group = transform_data(data.process(1, ['math4_silence.txt'], channels=channels, sample_rate=250, surrounding=surrounding))
    ...
    with tf.Session() as session:
        ...
        for epoch in range(num_epochs):
            ...
            if training_accuracy > 0.97:
                good_run = True
            if not good_run and epoch + 1 >= 100: #150
                break
