...
sequence_groups = transform_data(data.process_scrambled(labels, ['math6.txt'], channels=channels, sample_rate=250,
                                                        surrounding=surrounding, exclude=set([25, 99, 407])))

sequence_groups = data.transform.ica(sequence_groups, 10)
...
fig, axes = plt.subplots(len(sequence_groups), 1, figsize=(8, 10))
...
for i in range(len(sequence_groups)):
    avg1 = np.mean(sequence_groups[i], axis=0)
