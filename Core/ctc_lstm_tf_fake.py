...
#label_map = map(lambda n: tuple(np.random.randint(0, 3, n)), np.random.randint(2, 9, 1000))
label_map = map(lambda n: tuple((np.arange(n) % 3)[np.random.permutation(n)]), np.random.randint(3, 10, 1000))
#label_map = map(lambda n: tuple((np.arange(n) % 3)[np.random.permutation(n)]), np.random.randint(13, 25, 1000))
label_map = map(lambda seq: tuple(map(lambda x: x[0], groupby(seq))), label_map)
label_map = map(list, set(label_map))
def add_blanks(seq):
    result = [-1]
    for x in seq:
        result.append(x)
        result.append(-1)
    return result
#label_map = map(add_blanks, label_map)
#print label_map
print len(label_map)

num_samples_per_label_sequence = 10 #100
num_channels = 1
num_signal_repeats = 4
sequence_groups = [[None] * num_samples_per_label_sequence for _ in range(len(label_map))]
for i in range(np.shape(sequence_groups)[0]):
    for j in range(np.shape(sequence_groups)[1]):
        sequence_groups[i][j] = [[float(x)*10] * num_channels \
                                 for x in label_map[i] for k in range(np.random.randint(3, 10))]
sequence_groups = np.array(sequence_groups)

def remove_blanks(seq):
    seq = np.array(seq)
    return list(seq[np.where(seq > -1)])
#label_map = map(remove_blanks, label_map)
print label_map
#print len(label_map)

#for i in range(len(sequence_groups)):
#    for seq in sequence_groups[i]:
#        print label_map[i], seq
#    print
...
#lstm2 = tf.nn.rnn_cell.BasicLSTMCell(lstm_hidden_size)
#dropout2 = tf.nn.rnn_cell.DropoutWrapper(lstm2, 1.0-dropout_rate)

lstm_stack = tf.nn.rnn_cell.MultiRNNCell([dropout1])
#lstm_stack = tf.nn.rnn_cell.MultiRNNCell([dropout1, dropout2])

outputs, states = tf.nn.dynamic_rnn(lstm_stack, inputs, dtype=tf.float32)
...
#reshaped = tf.reshape(inputs, [-1, np.shape(train_sequences[0])[1]])
...
reshaped = tf.reshape(inputs, [-1, np.shape(train_sequences[0])[1]])
W = tf.Variable(tf.truncated_normal([np.shape(train_sequences[0])[1], num_classes], stddev = 0.1))
