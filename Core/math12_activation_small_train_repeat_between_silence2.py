...
sequence_groups = transform_data([data.flatten(data.process_scrambled(labels, ['math12.txt'], channels=channels, sample_rate=250,
                                                                      surrounding=surrounding, exclude=set([275, 396])))])

#silence_group = transform_data(data.process(1, ['math6_silence.txt'], channels=channels, sample_rate=250, surrounding=surrounding))
silence_group = transform_data(data.process_silence_between(['math12.txt'], channels=channels, sample_rate=250, surrounding=surrounding))
    ...
    with tf.Session() as session:
        ...
        for epoch in range(num_epochs):
            ...
            if validation_accuracy > max_validation_accuracy:
                model_name = 'math12_activation_small_model2.ckpt'
