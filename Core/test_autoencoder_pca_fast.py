...
with tf.Session() as session:
    ...
    saver.restore(session, 'autoencoder_' + str(window_size) + '_model.ckpt')
...
#        plt.show()
        ...
        lines = [axis.plot([],[], '-', c=colors[c])[0] for c in range(8)]
        axis.axis([0, window_size, -150, 150])
...
#            test_encoding = np.array(transformed[0])
            test_encoding = np.array(flat)
            value = 1000 * np.sin(5 * test_count * np.pi / 180.0)
            test_encoding[2] += value
            print value
            test_count += 1
            test_reconstructed = np.dot(np.array([test_encoding])[:,:num_components], pca.components_[:num_components,:])[0]
            test_reconstructed += mu
            reconstructed_output = session.run(outputs, {fc1: [test_reconstructed]})
            for c in range(8):
                lines[c].set_data(xs, reconstructed_output[0][:,c])
