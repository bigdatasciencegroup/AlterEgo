...
length_act = 75 #150, 600 DO NOT CHANGE
...
with tf.Session(graph=graph1) as session1:
    if test_model:
        ...
        saver1.restore(session1, 'math5_trigger_model.ckpt')
    with tf.Session(graph=graph2) as session2:
        if test_model:
            tf.global_variables_initializer().run()
            saver2.restore(session2, 'math5_activation_small2_model.ckpt')
...
#                trigger_labels = [6, 5, 4, 10, 0, 11, 2, 1, 4, 7, 11, 1, 10, 6, 4, 13, 7, 6, 6, 9, 3, 3, 3, 1, 4, 10, 3, 13, 1, 11, 1, 3, 0, 4, 10, 13, 4, 12, 5, 5, 11, 1, 13, 8, 1, 0, 5, 13, 0, 5, 11, 5, 11, 9, 11, 5, 6, 9, 6, 5, 13, 2, 5, 10, 5, 8, 3, 1, 3, 3, 10, 5, 5, 9, 2, 2, 9, 11, 12, 12, 8, 11, 11, 12, 6, 5, 8, 4, 5, 4, 8, 7, 5, 8, 12, 10, 8, 5, 8, 3, 7, 13, 5, 5, 8, 12, 5, 0, 8, 6, 8, 13, 8, 8, 4, 5, 4, 7, 12, 1, 5, 5, 5, 11, 11, 10, 5, 5, 6, 9, 10, 2, 2, 7, 8, 7, 13, 13, 7, 3]
#
#                if test_model:
#                    update_count += 1
#                    start = None
...
#                    activated = np.all(1-np.argmax(activation_output, axis=1) == 1)
                    ...
                    activation_windows = np.array(map(lambda x: x[0] > 0.9999, activation_output))
...
#                    activated = sum(activation_windows) >= len(activation_windows) - 2
                    ...
                    for i in range(len(activation_windows)):
                        if i >= 2 and np.all(activation_windows[i-2:i]) and not activation_windows[i]:
                            if 18 <= i - tmp_start and i < len(activation_windows) - 4: # and i - tmp_start <= 25
                    ...
                    activated = activated_start and activated_end and (activated_start - last_activated_start >= 4 
...
#                    best_guess = np.argmax(certainty_sums)
                    if activated:
                        ...
                        end = min(center+length//2, len(history))
                        start = end - length
                        sequence = history[start:end,channels] #TEST
                        test_feed = {inputs: [sequence], training: False}
                        test_output = session1.run(logits, test_feed)[0]
                        last_test_output = test_output
                        best_guess = np.argmax(test_output)
                        if time.time() - produced_timestamp > 1.0:
...
#        data.from_file.start('math5_test.txt',
#                             on_data, channels=channels, transform_fn=transform_data,
...
#                             override_step=45, sample_rate=250) # 45
        ...
        data.serial.start('/dev/tty.usbserial-DQ007UBV',
                          on_data, channels=channels, transform_fn=transform_data,
                          history_size=1500, shown_size=1200, override_step=95)