...
print word_syllable_map['teacher']
...
#print len(set(words))
words = set(filter(lambda w: w in word_phoneme_map and w in word_syllable_map, words))
#print len(words)
...
phoneme_sets = map(lambda y: sum(map(word_syllable_map.get, y), []), word_sets)
...
all_phonemes = np.unique(reduce(lambda a,b: a+b, phoneme_sets, []))
...
#    contains, counts = np.unique(word_syllable_map[word], return_counts=True)
#    print word_syllable_map[word]
#    print contains
#    print counts
#    print np.unique(word_syllable_map[word], axis=0, return_counts=True)
    contains_counts = {}
    for syllable in word_syllable_map[word]:
...
#    contains, counts = np.unique(phoneme_sets[i], return_counts=True)
    contains_counts = {}
    for syllable in phoneme_sets[i]:
        contains_counts[syllable] = contains_counts.get(syllable, 0) + 1
    contains = contains_counts.keys()
    counts = np.array(map(contains_counts.get, contains))
    phoneme_counts = np.array([0] * len(phoneme_map))
