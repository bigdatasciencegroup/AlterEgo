...
cnn_window_size = 1250 #1250
cnn_window_stride = 250 #250
num_channels = 8
...
conv1 = tf.layers.conv1d(reshaped, 512, 3, activation=tf.nn.relu, padding='valid')
conv1 = tf.layers.max_pooling1d(conv1, 2, strides=2)
conv2 = tf.layers.conv1d(conv1, 512, 3, activation=tf.nn.relu, padding='valid')
...
reshaped = tf.layers.dense(reshaped, fc_size, activation=tf.nn.relu,
    kernel_initializer=tf.initializers.truncated_normal(stddev=np.sqrt(2.0/(int(np.prod(dropout.shape[1:])) * fc_size))))
reshaped = tf.layers.dense(reshaped, fc_size, activation=tf.nn.relu,
...
#            batch_decoded = tf.sparse_tensor_to_dense(batch_decoded[0], default_value=-1).eval()
        ...
        print 'Validation:'
        for i in range(min(5000, len(validation_decoded))):
