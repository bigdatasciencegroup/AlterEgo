...
#sequence_groups = transform_data(data.digits_session_dependence_3_dataset(channels=range(1, 8)))
#for i in range(len(sequence_groups)):
##    sequence_groups[i] = sequence_groups[i][:30]
#    sequence_groups[i] = sequence_groups[i][:40]
...
with tf.Session() as session:
    ...
    for epoch in range(num_epochs):
        ...
        if validation_accuracy > max_validation_accuracy:
            save_path = saver.save(session, os.path.join(abs_path, 'cross_sess_model.ckpt'))
            print '\n\nModel saved:', save_path, '\n'
        max_validation_accuracy = max(validation_accuracy, max_validation_accuracy)
        
        if training_accuracy >= 0.90:
            good_run = True
        if epoch +1 >= 20 and not good_run:
            print '\n\n'
            sys.exit()
