...
def outside_fn(logits, loss, targets, sequence_lengths):
    ...
    targets_sparse = tf.SparseTensor(idx, tf.gather_nd(a_t, idx), tf.cast(tf.shape(a_t), tf.int64))
...
#    loss = tf.nn.ctc_loss(labels=targets_sparse, inputs=logits, sequence_length=sequence_lengths, time_major=True)
...
#    print decoded
    ...
    return logits, loss, #decoded, log_prob, error#, optimizer

#logits, loss, decoded, log_prob, error = tpu.rewrite(model_computation, model_inputs)
#logits, loss, decoded, log_prob, error, optimizer = model_computation(*model_inputs)
logits, loss = tpu.rewrite(model_computation, model_inputs)

#graph = tf.get_default_graph()
#filtered_ops = [op for op in tf.get_default_graph().get_operations() if graph.is_fetchable(op)]
#print filtered_ops

optimizer = tf.get_default_graph().get_operation_by_name('optimizer')

#log_prob = tf.get_default_graph().get_tensor_by_name('log_prob')
#error = tf.get_default_graph().get_tensor_by_name('error')
#decoded = tf.get_default_graph().get_tensor_by_name('decoded')
...
with tf.Session(tpu_grpc_url) as session:
    ...
    for epoch in range(num_epochs):
        ...
        for batch in range(num_training_batches):            
            ...
            batch_label_lengths = map(len, batch_labels)
            ...
            training_feed = {inputs: batch_sequences, targets: batch_labels,
                             sequence_lengths: batch_lengths, label_lengths: batch_label_lengths,
...
#            batch_decoded = batch_decoded.eval()
            batch_decoded = batch_decoded
