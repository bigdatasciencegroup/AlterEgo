...
with tf.Session() as session:
    if test_model:
        ...
        saver.restore(session, 'math12_trigger_model2.ckpt')
    ...
    def on_data(history, trigger_history, index_history, count, samples_per_update):
        ...
        if count - displayed > step:
            ...
            labels = [11, 6, 7, 14, 9, 5, 3, 10, 6, 12, 12, 12, 0, 11, 14, 7, 13, 12, 8, 7, 14, 2, 10, 13, 9, 12, 4, 5, 11, 14, 4, 5, 7, 8, 1, 6, 4, 10, 4, 2, 14, 3, 4, 13, 4, 8, 11, 10, 5, 6, 8, 11, 2, 4, 9, 13, 0, 10, 1, 12, 5, 6, 5, 13, 6, 3, 5, 3, 11, 9, 12, 6, 2, 8, 0, 14, 3, 14, 5, 11, 1, 7, 0, 13, 7, 2, 7, 7, 0, 14, 1, 10, 11, 10, 8, 13, 1, 3, 3, 1, 10, 2, 12, 2, 6, 3, 0, 8, 2, 0, 2, 8, 9, 0, 9, 1, 9, 1, 13, 4]
...
#                            os.system('say ' + str(word_map[selected]) + ' &')
    ...
    data.from_file.start('math12_test.txt',
                      on_data, channels=channels, transform_fn=transform_data,
                      history_size=1500, shown_size=1200, override_step=65, sample_rate=250, speed=1.0)
