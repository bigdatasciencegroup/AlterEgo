...
sequence2 = data.transform.bandpass_filter(sequence, low_freq, high_freq, sample_rate, order=order)
fft2 = np.abs(data.transform.fft(sequence2)) / len(sequence2)
sequence3 = data.transform.fft(sequence)
sequence3 = data.transform.fft_frequency_cutoff(sequence3, low_freq, high_freq, sample_rate)
sequence3 = np.real(data.transform.ifft(sequence3))
fft3 = np.abs(data.transform.fft(sequence3)) / len(sequence3)
...
plt.subplots_adjust(left=0.05, right=0.995, bottom=0.075, top=0.96, hspace=0.45, wspace=0.15)
axes[0][0].set_title('Unfiltered Signal')
axes[0][0].set_ylabel(u'Signal (\u03bcV)')
axes[1][0].set_title('Bandpassed Signal (0.5-8 Hz Butterworth)')
axes[1][0].set_ylabel(u'Signal (\u03bcV)')
axes[2][0].set_title('Hard-Filtered Signal (0.5-8 Hz via Modified Frequency Domain)')
axes[2][0].set_ylabel(u'Signal (\u03bcV)')
axes[2][0].set_xlabel('Sample')
axes[0][1].set_title(u'FFT of Unfiltered Signal (\u2264150Hz)')
axes[0][1].set_ylabel('Magnitude')
axes[1][1].set_title(u'FFT of Bandpassed Signal (\u2264150Hz)')
axes[1][1].set_ylabel('Magnitude')
axes[2][1].set_title(u'FFT of Hard-Filtered Signal (\u2264150Hz)')
...
for i in range(len(channels)):
    ...
    axes[2][0].plot(sequence3[:,i], c=colors[channels[i]], alpha=1.0, lw=1.0)
    axes[0][1].plot(np.arange(541) * sample_rate/length, fft[:541,i], # len(fft)//2+1
                    c=colors[channels[i]], alpha=1.0, lw=1.0)
    axes[1][1].plot(np.arange(541) * sample_rate/length, fft2[:541,i],
                    c=colors[channels[i]], alpha=1.0, lw=1.0)
    axes[2][1].plot(np.arange(541) * sample_rate/length, fft3[:541,i],
                    c=colors[channels[i]], alpha=1.0, lw=1.0)
plt.savefig('figure_bandpass_correction.png')
plt.show()
