...
word_map = ['i', 'am', 'you', 'are', 'the', 'want', 'need', 'cold', 'hot', 'food', 'where', 'what', 'how', 'feeling', 'doing', 'tired', 'water', 'hungry', 'thirsty', 'hello']
...
num_classes = 20 #len(filter(lambda x: '.txt' in x, os.listdir('patient_data')))
...
with tf.Session(graph=graph1) as session1:
    if test_model:
        ...
        saver1.restore(session1, 'words20_trigger_model.ckpt')
    with tf.Session(graph=graph2) as session2:
        if test_model:
            tf.global_variables_initializer().run()
            saver2.restore(session2, 'words20_activation_small_model.ckpt')
        ...
        def on_data(history, trigger_history, index_history, count, samples_per_update):
            ...
            if count - displayed > step:
                ...
                labels = [3, 5, 1, 11, 10, 2, 5, 1, 5, 12, 11, 11, 9, 13, 7, 11, 8, 6, 13, 12, 19, 4, 6, 0, 10, 3, 2, 18, 17, 11, 15, 12, 13, 16, 7, 12, 5, 8, 4, 7, 16, 13, 13, 19, 1, 14, 9, 3, 6, 14, 17, 2, 9, 19, 1, 9, 2, 6, 11, 0, 5, 17, 4, 4, 15, 18, 19, 18, 15, 0, 15, 14, 0, 0, 14, 8, 6, 10, 11, 5, 7, 9, 15, 12, 13, 11, 1, 4, 4, 16, 4, 10, 15, 19, 3, 2, 8, 1, 16, 13, 14, 17, 10, 12, 18, 14, 8, 1, 10, 5, 13, 10, 6, 3, 9, 5, 14, 3, 19, 15, 18, 6, 2, 8, 0, 18, 3, 19, 15, 14, 1, 7, 16, 0, 7, 2, 7, 7, 0, 19, 10, 12, 8, 6, 17, 17, 18, 12, 2, 3, 16, 8, 16, 18, 9, 16, 17, 9, 17, 4]
...
#                print 'Predicted\t', predictions
        ...
        data.from_file.start('words20_test.txt',
