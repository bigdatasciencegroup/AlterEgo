...
validation_sequences = transform_data(data.process_scrambled_flattened(labels, ['math1_test.txt'], channels=channels, sample_rate=250))
...
validation_sequences = np.array(list(validation_sequences) + list(validation_silence_group[0]))
validation_sequence_groups = np.array(list(validation_sequence_groups) + list(validation_silence_group))

labels += [len(training_sequence_groups)-1] * len(validation_silence_group[0])
print labels

lengths = map(len, validation_sequences)
print min(lengths), np.mean(lengths), max(lengths)
...
length = 410 #410 DO NOT CHANGE
training_sequence_groups = data.transform.pad_truncate(training_sequence_groups, length)
validation_sequences = data.transform.pad_truncate(validation_sequences, length)
...
val_sequences, val_labels = validation_sequences, labels
...
num_epochs = 1 # Fixed, just predicting
batch_size = 50
...
with tf.Session() as session:
    ...
    for epoch in range(num_epochs):
        ...
        for batch in range(num_training_batches):            
            ...
            batch_loss, batch_output = session.run([loss, logits], training_feed)
        ...
        test_output = list(np.argmax(val_output, axis=1))
