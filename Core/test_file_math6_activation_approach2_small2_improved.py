...
with tf.Session(graph=graph1) as session1:
    if test_model:
        ...
        saver1.restore(session1, 'math6_trigger_model.ckpt')
    with tf.Session(graph=graph2) as session2:
        if test_model:
            tf.global_variables_initializer().run()
            saver2.restore(session2, 'math6_activation_small_model.ckpt')
    ...
    #                            predictions.append(selected)
                ...
                trigger_labels = [11, 9, 7, 1, 13, 5, 14, 4, 2, 12, 9, 0, 10, 5, 7, 4, 9, 6, 3, 3, 9, 8, 7, 4, 12, 9, 3, 2, 7, 14, 9, 13, 6, 12, 11, 4, 5, 8, 5, 10, 11, 7, 3, 10, 5, 11, 1, 0, 4, 5, 3, 3, 11, 13, 10, 13, 12, 8, 12, 13, 1, 11, 12, 1, 0, 10, 14, 12, 11, 10, 5, 14, 13, 11, 6, 7, 11, 9, 10, 8, 1, 3, 3, 5, 3, 8, 11, 14, 2, 2, 6, 0, 13, 9, 10, 14, 8, 14, 10, 10, 1, 8, 4, 8, 5, 2, 7, 4, 2, 13, 12, 14, 4, 2, 7, 0, 14, 3, 14, 10, 4, 5, 1, 8, 0, 8, 13, 0, 0, 5, 0, 6, 6, 0, 8, 9, 7, 5, 10, 0, 2, 13, 7, 12, 7, 13, 13, 4, 14]
...
#                    activated = sum(activation_windows) >= len(activation_windows) - 2
                    ...
                    for i in range(len(activation_windows)):
                        if i >= 4 and np.all(activation_windows[i-4:i]) and not activation_windows[i]:
                            if 10 <= i - tmp_start and i < len(activation_windows) - 2: # and i - tmp_start <= 25
