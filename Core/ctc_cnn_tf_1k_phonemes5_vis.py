...
sequence_groups = data.phonemes_5_sentences_dataset(include_surrounding=False)
...
label_map = [[23, 9, 23, 11, 7, 10, 15, 2],
             [7, 9, 17, 6, 5, 22, 11, 16, 11, 21, 15, 2],
             [23, 1, 21, 4, 3, 15, 10, 14, 11, 16, 12, 23, 10],
             [7, 10, 23, 3, 19, 0, 8, 3, 6, 5],
             [19, 9, 13, 23, 12, 21, 20, 12, 21, 4, 11, 14, 18, 1, 14]]
#label_map = [[7, 9, 17, 6, 5, 22, 11, 16, 11, 21, 15, 2], #ERRONEOUS LABEL MAP
#             [23, 1, 21, 4, 3, 15, 10, 14, 11, 16, 12, 23, 10],
#             [7, 10, 23, 3, 19, 0, 8, 3, 6, 5],
#             [19, 9, 13, 23, 12, 21, 20, 12, 21, 4, 11, 14, 18, 1, 14],
#             [23, 9, 23, 11, 7, 10, 15, 2]]
...
cnn_window_size = 600 * sample_rate // 1000 #600 #1250
cnn_window_stride = 100 * sample_rate // 1000 #100 #250
num_channels = 4
...
conv1 = tf.layers.conv1d(reshaped, 512, 3, activation=tf.nn.relu, padding='valid', name='conv1')
conv1 = tf.layers.max_pooling1d(conv1, 2, strides=2)
conv2 = tf.layers.conv1d(conv1, 512, 3, activation=tf.nn.relu, padding='valid', name='conv2')
...
vis_conv1 = tf.layers.conv1d(vis_inputs, 512, 3, activation=tf.nn.relu, padding='valid', name='conv1', reuse=True)
vis_conv1 = tf.layers.max_pooling1d(vis_conv1, 2, strides=2)
vis_conv2 = tf.layers.conv1d(vis_conv1, 512, 3, activation=tf.nn.relu, padding='valid', name='conv2', reuse=True)
