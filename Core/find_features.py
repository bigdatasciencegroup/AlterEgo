...
notch1 = 60 #59.9
num_times = 3 #2
freqs = np.arange(1, sample_rate/(2. * notch1)) * notch1
...
#sequence_groups1 = data.transform.bandpass_filter(sequence_groups, low, high, sample_rate, order=order)
fft = data.transform.fft(sequence_groups)
fft = data.transform.fft_frequency_cutoff(fft, low, high, sample_rate)
sequence_groups1 = data.transform.ifft(fft)

sin_kernel = normalize_kernel(np.sin(np.arange(250)/250. * 1*np.pi), subtract_mean=True)
sequence_groups2 = data.transform.correlate(sequence_groups1, sin_kernel)

#sequence_groups = np.array([np.array([np.concatenate([sequence_groups1[i][j], sequence_groups2[i][j]], axis=1) for j in range(len(sequence_groups[i]))]) for i in range(len(sequence_groups))])
#sequence_groups = sequence_groups1
sequence_groups = sequence_groups2
...
plt.subplots_adjust(left=0.075, right=0.975, bottom=0.08, top=0.95, hspace=0.45)
...
plt.title('Example Cluster Representation (num_clusters='+str(num_clusters)+', window_size='+str(window_size)+')')
...
for i in range(num_clusters):
    ...
    plt.title('Cluster ' + str(i) + ' (count: '+str(cluster_counts[i])+')')
