...
def dataset(**kwargs):
    ...
    files = map(lambda x: patient_dir + '/' + x, filter(lambda x: '.txt' in x, os.listdir('patient_data')))
    files.sort()
    print files
...
####################
#### Model (DO NOT CHANGE, MUST BE SAME AS patient_test_serial.py)
learning_rate = 1e-4
dropout_rate = 0.4
...
with tf.Session() as session:
    ...
    for epoch in range(num_epochs):
        ...
        if validation_accuracy > max_validation_accuracy:
            save_path = saver.save(session, os.path.join(abs_path, 'patient_model.ckpt'))
