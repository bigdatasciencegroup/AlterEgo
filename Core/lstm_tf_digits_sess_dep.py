...
fc1 = tf.layers.dense(reshaped, 512, activation=tf.nn.relu,
            kernel_initializer=tf.initializers.truncated_normal(stddev=np.sqrt(2.0/(2 * lstm_hidden_size * 512))))
logits = tf.layers.dense(fc1, num_classes, activation=tf.nn.softmax,
            kernel_initializer=tf.initializers.truncated_normal(stddev=np.sqrt(2.0/(512 * num_classes))))
