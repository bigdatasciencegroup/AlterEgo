...
#loss = tf.reduce_mean(tf.multiply(tf.nn.softmax_cross_entropy_with_logits(logits=logits_t, labels=targets), weights))
loss = tf.nn.ctc_loss(labels=targets, inputs=logits_t, sequence_length=sequence_lengths, time_major=True)
...
#decoded, log_prob = tf.nn.ctc_greedy_decoder(logits_t, sequence_lengths)
ctc_output = tf.nn.ctc_beam_search_decoder(logits_t, sequence_lengths, top_paths=10) # 10
...
with tf.Session() as session:
    ...
    saver.restore(session, 'ctc_cnn_sen200_test_model.ckpt')
...
#        permutation = np.random.permutation(num_validation_samples)
        permutation = range(num_validation_samples)
...
#            print np.shape(batch_sequences)
            ...
            batch_loss, batch_error, batch_decoded, batch_log_prob, batch_logits = \
                                            session.run([loss, error, decoded, log_prob, softmax_logits], validation_feed)
            validation_loss += batch_loss * len(indices)
            validation_error += batch_error * len(indices)
            
            validation_logits.extend(batch_logits)
...
#            batch_error_selected = np.transpose(np.reshape(batch_error_selected, [top_k, -1]))
            ...
            for i in range(len(batch_error_selected)):
                error_rates.append(batch_error_selected[i, 0])
...
#                            np.where(training_decoded[i][pairs[j][1]] > -1)]])
        print 'Validation:'
        wpm_error_pairs = []
        for i in range(min(10000, len(validation_decoded))):
#            print val_labels[i], ' => ', validation_decoded[i][np.where(validation_decoded[i] > -1)]
            wpm = 60.0 / (float(len(val_sequences[i])-2*surrounding) / 250 / len(val_labels[i]))
            print len(val_sequences[i])
            print wpm
            print error_rates[i]
            wpm_error_pairs.append((wpm, error_rates[i]))
        ...
        print
        print wpm_error_pairs
        wpms, errors = zip(*wpm_error_pairs)
        plt.scatter(wpms, errors)
        plt.plot(np.unique(wpms), np.poly1d(np.polyfit(wpms, errors, 1))(np.unique(wpms)))
        plt.show()
        min_wpm = min(wpms)
        max_wpm = max(wpms)
#        wpms, errors = zip(*[(i, np.mean(map(lambda x: x[1], filter(lambda x: i-6 <= x[0] < i+6, wpm_error_pairs)))) for i in np.arange(min_wpm, max_wpm, 0.25)]) #6

        wpms, errors = zip(*[(i, np.mean(map(lambda x: x[1], filter(lambda x: i-5 <= x[0] < i+5, wpm_error_pairs)))) for i in np.arange(min_wpm, max_wpm+0.5, 0.5)]) #6
        plt.scatter(wpms, errors)
        
        wpms, errors = zip(*[(i, np.mean(map(lambda x: x[1], filter(lambda x: i-5 <= x[0] < i+5, wpm_error_pairs)))) for i in np.arange(min_wpm+1.0, max_wpm+5, 5)]) #6
        plt.scatter(wpms, errors)
        
#        wpms, errors = zip(*[(i, map(lambda x: x[1], filter(lambda x: i-5 <= x[0] < i+5, wpm_error_pairs))) for i in np.arange(min_wpm, max_wpm, 5)]) #6
#        plt.boxplot(errors, positions=wpms)

        for i in range(len(wpms)):
            x, y = wpms[i], errors[i]
            plt.plot([x-5, x+5], [y, y], 'k')
            plt.plot([x-5, x-5], [y-0.002, y+0.002], 'k')
            plt.plot([x+5, x+5], [y-0.002, y+0.002], 'k')
        plt.show()
        ...
        for i in range(5):
            plt.figure()
            print ' '.join(words[val_labels[i]])
            plt.plot(np.transpose(validation_logits[i]))
        plt.show()
