...
with tf.Session(graph=graph1) as session1:
    if test_model:
        ...
        saver1.restore(session1, 'math8_trigger_model' + sys.argv[2] + '.ckpt')
    with tf.Session(graph=graph2) as session2:
        if test_model:
            tf.global_variables_initializer().run()
            saver2.restore(session2, 'math8_activation_small_model' + sys.argv[2] + '.ckpt')
    ...
    #                            predictions.append(selected)
                ...
                trigger_labels = [11, 6, 7, 14, 9, 5, 3, 10, 6, 12, 12, 12, 0, 11, 14, 7, 5, 9, 8, 7, 14, 2, 10, 13, 9, 12, 4, 5, 11, 14, 5, 5, 7, 8, 1, 6, 4, 10, 4, 2, 14, 3, 4, 13, 4, 1, 6, 10, 4, 6, 8, 11, 2, 4, 9, 13, 0, 10, 1, 12, 5, 6, 13, 13, 6, 3, 5, 3, 11, 9, 12, 6, 2, 8, 3, 14, 3, 14, 5, 11, 10, 7, 0, 13, 7, 2, 7, 7, 0, 14, 1, 10, 11, 10, 8, 13, 9, 3, 3, 9, 10, 2, 12, 2, 6, 3, 0, 5, 9, 0, 2, 8, 9, 0, 9, 9, 13, 4, 13, 4]
