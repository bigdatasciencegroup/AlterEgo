...
with tf.Session(graph=graph1) as session1:
    if test_model:
        ...
        saver1.restore(session1, 'math25_trigger_model.ckpt')
    with tf.Session(graph=graph2) as session2:
        if test_model:
            tf.global_variables_initializer().run()
            saver2.restore(session2, 'math25_activation_small_model.ckpt')
...
#                    sequence_act = sequence
                    ...
                    activation_windows = np.array(map(lambda x: x[0] > 0.98, activation_output)) #0.98
