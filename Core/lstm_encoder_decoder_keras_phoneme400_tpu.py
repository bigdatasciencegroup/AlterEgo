...
for i in range(num_encoder_layers):
    inputs = encoder_inputs if i == 0 else encoder_lstm
    if i < num_encoder_layers - 1:
        encoder_lstm = LSTM(latent_dim, return_sequences=True, return_state=True)(inputs) #CuDNNLSTM
    else:
        encoder_lstm = LSTM(latent_dim, return_state=True)(inputs)
encoder_outputs, state_h, state_c = encoder_lstm
encoder_states = [state_h, state_c]

decoder_inputs = Input(shape=(max_labels_length, num_classes))
...
tpu_grpc = TPUClusterResolver(tpu=[os.environ['TPU_NAME']])
tpu_model = tf.contrib.tpu.keras_to_tpu_model(model, strategy=tf.contrib.tpu.TPUDistributionStrategy(tpu_grpc))
...
fetches = [tf.assign(training_callbacks.batch_targets, tpu_model.targets[0], validate_shape=False),
           tf.assign(training_callbacks.batch_outputs, tpu_model.outputs[0], validate_shape=False)]
tpu_model._function_kwargs = {'fetches': fetches}

tpu_model.fit([train_sequences, train_labels[:,:-1,:]], train_labels[:,1:,:],
