...
double_protocol = lights
#double_protocol = True
        ...
        if activated:
            ...
            if max_activation_output <= 0.90:
                ...
                if now - timestamp > wait_time:
                    ...
                    timestamp = now
                    if (last_prediction == prediction or not double_protocol) and now - prediction_timestamp <= prediction_time_limit:
#                        os.system('say ' + str(prediction) + ' &')
                        if tetris: control_tetris(prediction)
                        if lights: control_phue(prediction)
                        prediction = None
...
data.serial.start('/dev/tty.usbserial-DQ007UBV', on_data, override_step=30) #25
